...
import { Calendar, Plus, Clock, CheckCircle, XCircle, AlertCircle, User, Search } from 'lucide-react';
...
const LeaveManagementPage: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [requests, balances, employeeList] = await Promise.all([
        ...
        currentUser ? leaveService.getLeaveBalances(currentUser.uid) : Promise.resolve([]),
  ...
  const handleCreateRequest = async (requestData: CreateLeaveFormData) => {
    try {
      ...
      const totalDays = leaveService.calculateLeaveDays(requestData.startDate, requestData.endDate);

      const newLeaveRequest: Omit<LeaveRequest, 'id' | 'createdAt' | 'updatedAt'> = {
  ...
  const filteredRequests = leaveRequests.filter(request => {
    if (filters.searchTerm && !request.employeeName.toLowerCase().includes(filters.searchTerm.toLowerCase())) {
      return false;
    }
    if (filters.status !== 'All Statuses' && request.status !== filters.status) {
      return false;
    }
    if (filters.leaveType !== 'All Types' && request.leaveType !== filters.leaveType) {
      return false;
    }
    return true;
  });
          ...
          <div className="border-b border-gray-200 mb-6">
            <nav className="-mb-px flex space-x-8">
              {([
                ...
                { id: 'requests', label: 'Leave Requests', icon: <Clock className="w-5 h-5" /> },
                { id: 'balances', label: 'Leave Balances', icon: <Calendar className="w-5 h-5" /> },
